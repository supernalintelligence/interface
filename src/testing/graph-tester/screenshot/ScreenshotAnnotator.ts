...
export class ScreenshotAnnotator {
  private config: ScreenshotAnnotatorConfig;
   ...
   */
  async capture(page: Page, route: string, viewportName?: string): Promise<AnnotatedScreenshot> {
   ...
   */
  private async extractAnnotations(page: Page): Promise<ElementAnnotation[]> {
    const config = this.config;

    // Single-pass DOM traversal in browser context
    const rawAnnotations = await page.evaluate(
      ({ includeInvisible, minElementSize, detectNavigation }) => {
        const annotations: any[] = [];
        const elementMap = new Map<Element, string>(); // Element -> ID mapping
        let idCounter = 0;
        ...
        for (const element of allElements) {
          ...
          let navigationTarget: any = undefined;
    ...
    return rawAnnotations as ElementAnnotation[];
  }
   ...
   */
  private generateScreenshotFilename(route: string, viewportName?: string): string {
